...
func _on_robot_ready():
	print("robot ready")
	tick_timer.one_shot = false
	add_child(tick_timer)
	tick_timer.start(0.5)
...
func _on_button_pressed():
	code_window.reset_console()
	if is_instance_valid(robot):
		robot.queue_free()
	var source = code_window.get_source_code()
	...
	source = replace_print_calls_in_script(code_window.name,source)
	...
	robot.__reset__()
	robot.ready.connect(_on_robot_ready)
	robot.move_requested.connect(_on_robot_move_requested)
	paused = false
	add_child(robot)
