...
const WIDTH:int = 6
const HEIGHT:int = 6
const ROCK_COORDS = [Vector2i(0,2),Vector2i(3,2), Vector2i(5, 3)]
const WATER_COORDS = [Vector2i(1,3), Vector2i(1,4), Vector2i(2, 3), Vector2i(2, 4)]
...
# vis representation of farm at start


func before_each():
	#var farm_model = farm_model_generator()
	#var farm_view = FarmView.new()
	#farm_view.plot_farm(farm_model)
	level = level_scene.instantiate() as Level
	add_child(level)
	
func set_level(file_path:String):
	var level_skeleton = FileAccess.open(file_path, FileAccess.READ)
...
func farm_model_generator():
	var farm_model = FarmModel.new(WIDTH, HEIGHT)
	for i in ROCK_COORDS:
		assert_true(farm_model.is_empty(i))
		farm_model.add_farm_item(Obstacle.ROCK(), i)
		
		assert_false(farm_model.is_empty(i))
		assert_true(farm_model.is_obstacle(i))
	
	for i in WATER_COORDS:
		assert_true(farm_model.is_empty(i))
		farm_model.add_farm_item(Obstacle.WATER(), i)
...
func test_plant():
	set_level(PLANT_LEVEL)
	watch_signals(level.farm)
	level = level as Level
	
	var source = FileAccess.get_file_as_string(PLANT_SCRIPT)
	...
	#level.interpreter_client.load_source(level.window.get_source_code())
	#level.interpreter_client.start()
	#
	#level._on_timer_tick()
	#level._on_timer_tick()
	#level._on_timer_tick()
	#level._on_timer_tick()
...
func test_premature_harvest():
	set_level(HARVEST_LEVEL)
	watch_signals(level.farm)
	level = level as Level
	
	var source = FileAccess.get_file_as_string(PREMATURE_HARVEST_SCRIPT)
...
func test_mature_harvest():
	...
	var source = FileAccess.get_file_as_string(MATURE_HARVEST_SCRIPT)
...
func test_move():
	set_level(MOVE_LEVEL)
	watch_signals(level.farm)
	level = level as Level
	
	var source = FileAccess.get_file_as_string(MOVE_SCRIPT)
...
func test_reset():
	...
	source = FileAccess.get_file_as_string("")
	level.window.code_edit.text = source
	level._on_window_run_button_pressed()
	
	# Harvestables are reset
	assert_eq(level.farm.get_harvestables(), {})
	# farm model is clear
